#!/usr/bin/env python3
"""
Best-Fit Contiguous Scheduler

Assigns blocks to slots that provide the tightest fit, minimizing fragmentation.
Uses contiguous spanning when needed.

Strategy: For each block, find the slot that leaves minimum residual capacity
after assignment (best fit to avoid wasting space).
"""
...
def best_fit_contiguous(blocks, strategies, carbon, delta, error_threshold,
                       slot_duration_minutes, parallelism, capacity_tiers=None):
    """
    Best-fit scheduler with contiguous spanning.
    
    For each block:
    1. Find cheapest strategy that respects error budget
    2. Assign to slot that minimizes wasted capacity (tightest fit)
    ...
    for block_idx, block in enumerate(blocks):
        ...
        # Find best (strategy, slot) combination
        best_cost = float('inf')
        best_strategy_idx = None
        best_start_slot = None
        best_slots_used = None
        best_time_per_slot = None
        best_waste = float('inf')  # Track wasted capacity for best-fit
        ...
        for strategy_idx, strategy in enumerate(strategies):
            ...
            for start_slot in range(deadline + 1):
                ...
                # Calculate wasted capacity (best-fit metric)
                # Sum of residual capacity in slots used after this assignment
                waste = 0.0
                for slot_idx, time_used, _ in slots_used:
                    waste += residuals[slot_idx] - time_used
                
                # Best-fit: Choose assignment with minimum waste
                # (or minimum cost if waste is similar)
                if (waste < best_waste) or (abs(waste - best_waste) < 1e-6 and cost < best_cost):
                    best_waste = waste
                    best_cost = cost
        ...
        if best_strategy_idx is None:
            print(f"ERROR: No feasible assignment for block {block_idx}", 
                  file=sys.stderr)
...
def main():
    """Example usage"""
    if len(sys.argv) < 8:
        print("Usage: python best_fit_cont.py <blocks_csv> <strategies_csv> <carbon_csv> "
    ...
    # Run best-fit
    cost, error, assignments, residuals = best_fit_contiguous(
