#!/usr/bin/env python3
"""
First-Fit Contiguous Scheduler

Simple first-fit strategy that assigns each block to the first available slot
with sufficient capacity, using contiguous spanning if needed.

This is a fast baseline that prioritizes earliness over cost optimization.
"""
...
def first_fit_contiguous(blocks, strategies, carbon, delta, error_threshold,
                        slot_duration_minutes, parallelism, capacity_tiers=None):
    """
    First-fit scheduler with contiguous spanning.
    
    For each block:
    1. Find cheapest strategy that respects error budget
    2. Assign to first slot where it fits (possibly spanning)
    ...
    for block_idx, block in enumerate(blocks):
        ...
        # Find cheapest strategy that respects error budget
        ...
        for strategy_idx, strategy in enumerate(strategies):
            ...
            # Estimate cost (just for strategy selection, not placement)
            # Use average carbon as rough estimate
            avg_carbon = sum(carbon[:deadline+1]) / (deadline + 1)
            estimated_cost = avg_carbon * strategy['duration'] * block_size / parallelism
            
            if estimated_cost < best_strategy_cost:
                best_strategy_cost = estimated_cost
        ...
        # Now find first slot where this strategy fits
        ...
        for start_slot in range(deadline + 1):
            ...
            if feasible:
                # Found first fit!
                apply_assignment(residuals, time_per_slot)
                assignments.append((block_idx, best_strategy_idx, start_slot, slots_used))
                
                total_cost += cost
                cumulative_error += strategy['error'] * block_size
                cumulative_requests += block_size
                found = True
                break
        
        if not found:
            print(f"ERROR: No feasible slot for block {block_idx} with strategy {best_strategy_idx}",
                  file=sys.stderr)
            return None, None, None, None
...
def main():
    """Example usage"""
    if len(sys.argv) < 8:
        print("Usage: python first_fit_cont.py <blocks_csv> <strategies_csv> <carbon_csv> "
    ...
    # Run first-fit
    cost, error, assignments, residuals = first_fit_contiguous(
