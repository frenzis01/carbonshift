...
def main(input_requests,input_strategies,input_co2,delta,input_beta,input_ratio,error,output_assignment):
    num_slots               = delta                                   # Data          
    ...
    ratio_beta              = import_input_beta(input_ratio)
    
    assert (delta <= len(carbon) and delta <= len(requests)), "Provide a carbon intensity per slot and at least one request"
    # for delta > the provided slot parameter,
    # raise an exception since the code will not work properly, as it is not designed to handle such cases.
    # ------------------------

    """
    # NOTE: single requests
    ...
    """
    # NOTE: requests per each block of requests 
    req_actual,req_forecast = len(requests[0])*BETA[0],0
    for index,k in enumerate(requests,start=0):            # NOTE: 0 <= deadline < delta
        req_forecast += len(k)*BETA[index]
    req_forecast -= req_actual
    """
    ...
    # (2) request 'k' has exactly one slot 'i' and one strategy 'j' within deadline < window
    for z in all_slots:
        for k in requests[z]:   
            model.add_exactly_one(assignment[(i,j,k['id'])] for i in k['interval'] for j in all_strategies)

    # (3) average precision for actual requests R[0] is at least error threshold times the actual beta
    max_weighted_error_threshold_actual = error_threshold * beta[0] 
    ...
    # to simplify (4) and objective function, 
    # the division part is pre-computed in the input_ratio.csv file 
    # in which it is assured that it is an integer value
    sum_forecast = 0
    for r in range(1, num_slots):
        sum_forecast += beta[r] * ratio_beta[r]

    # (4) average precision for forecasted requests R[1, Delta-1] is at least error threshold times the forecasted beta
    max_weighted_error_threshold_forecast = error_threshold * sum_forecast 
    model.Add(max_weighted_error_threshold_forecast >= sum(
        assignment[(i,j,k['id'])] * strategies[j]['error'] * ratio_beta[z] 
        for j in all_strategies
    ...
    model.minimize(                                         # OBJECTIVE: minimize emission 
        sum(            
            assignment[(i,j,k['id'])] * emissions(carbon[i], strategies[j]['duration']) * ratio_beta[z] 
            for j in all_strategies
            for z in range(0, num_slots)        # 2: for each block of requests
            for k in requests[z]
            for i in k['interval']          # 1: minimize within its living slots
        )      
    )
    ...
    if status in [cp_model.UNKNOWN, cp_model.MODEL_INVALID, cp_model.FEASIBLE, 
        ...
        # NOTE: comment out the print statement to avoid filling the RAM
...
parser.add_argument('input_ratio', type=str, help='File with the pre-computed ratio between the requests and the number of blocks per each slot')
...
main(args.input_requests,args.input_strategies,args.input_co2,args.delta,args.input_beta,args.input_ratio,args.error,args.output_assignment)
