...
# Fill full blocks
def decrease_assignment(sum_list, assignment, ratio_slot):
    count_blocks = 0
...
def clear_file(OUTPUT_TIME):#, OUTPUT_COMPRESSION):
    ...
    #with open(OUTPUT_COMPRESSION, "w") as file_out:
    #    file_out.write(f"")                
    #    file_out.write(f"all_requests,delta,compression_time\n")

def clear_file_greedy(OUTPUT_TIME):
    with open(OUTPUT_TIME, "w") as file_out:                
        file_out.write(f"")                 
        file_out.write(f"all_requests,computing_time,strategy,all_emissions,slot_emissions,avg_errors,iteration\n")
        file_out.flush()

def prepare_file(d, beta, ratio, N_REQUESTS, INPUT_FILE, OUTPUT_FILE): #OUTPUT_COMPRESSION, 
            
    assignment, sum_all = [],[]                         # store the assignment and the sum of deadlines
    #compression = 0                                     # ratio and compression for actual slot
    ...
    with open(INPUT_FILE, "r", buffering=1) as file_in: # buffered reader with chunk size               
        ...
        if line:
            #START = float(subprocess.check_output("date +%s.%N", shell=True).decode().strip()) #time.time() 
            ...
            """
            END = float(subprocess.check_output("date +%s.%N", shell=True).decode().strip()) #time.time()
            diff = END - START #time.time() - start_time
            if diff > 300:  # 300 seconds = 5 minutes
                print(f"Skipping the reading due to elapsed timeout for requests {N_REQUESTS}")
                sys.stdout.flush()
            compression += diff
            """
    ...
    #with open(OUTPUT_COMPRESSION, "a") as file_out:
    #    file_out.write(f"{N_REQUESTS},{d},{compression}\n")
